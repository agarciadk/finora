...
- Loading spinners (`lucide-react`) on the login and logout buttons, and other in-flight actions, for clearer feedback while a request is pending. The initial load is also covered end-to-end: a pure HTML/CSS spinner is baked into `index.html` for the pre-hydration gap, and `AuthProvider` shows a full-screen `LoadingSpinner` while it checks for an existing session, so authenticated users never see the login page flash before the dashboard. Every authenticated route is code-split (`React.lazy`); clicking a Sidebar link preloads that route's chunk and shows a spinner over the content area (Sidebar/header stay visible, and the clicked link is highlighted immediately) instead of freezing the UI mid-navigation on a slow connection.

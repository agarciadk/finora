...
- Dedicated Categorías page for managing categories (name, type, edit/delete). Deleting a category is always safe: a single Prisma transaction finds or creates a fallback "Otros" category matching its type and reassigns every transaction/budget pointing at the deleted category before soft-deleting it, so there are never orphan records.

...
- **Env-driven timeouts, frontend**: `VITE_IDLE_WARNING_MINUTES`/`VITE_IDLE_LOGOUT_MINUTES` (default 14/15) now drive `useIdleLogout`'s defaults and `useSessionHeartbeat`'s activity window via a new shared `apps/web/src/lib/idle-config.ts`, so both can be shortened together for manual/e2e testing without touching hook code.
...
- **Conditional session heartbeat**: `use-session-heartbeat.ts` no longer refreshes the access token unconditionally whenever it's about to expire. It previously used its own hardcoded 5-minute "recently active" window, independent from (and much longer than) the idle-warning system's configurable threshold - during manual testing with shortened timeouts, this let the heartbeat keep sliding the session well past the point the user should have been considered idle, so the `IdleWarningModal` never got a chance to appear. The heartbeat's activity check now defaults to the SAME threshold as `useIdleLogout`'s `warningTimeoutMs` (`IDLE_WARNING_TIMEOUT_MS`, see `lib/idle-config.ts`), so both systems always agree on what counts as "still active": once the user has been idle longer than that, the heartbeat backs off and lets the token expire naturally, handing control to the idle-warning flow.

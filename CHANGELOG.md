...
- `lib/session-events.ts` gained a second event, `onSessionRefreshed`, so `AuthProvider` can keep `user.expiresAt` current after any successful reactive silent refresh, without `lib/api.ts` touching React state directly.
- New e2e suite `apps/web/e2e/tests/session-sync.spec.ts`: verifies `expiresAt` reflects the real ~5 minute access token lifespan, and that `/auth/refresh` rotates the access token, the (rotating) refresh token, and its "remember me" expiry strictly forward.
- **Env-driven timeouts, frontend**: `VITE_IDLE_WARNING_MINUTES`/`VITE_IDLE_LOGOUT_MINUTES` (default 14/15) now drive `useIdleLogout`'s defaults via a new shared `apps/web/src/lib/idle-config.ts`, so both can be shortened for manual/e2e testing without touching hook code.
- **Env-driven timeouts, backend**: `@nestjs/config`'s `ConfigService` (registered globally in `AppModule`) plus a new `AuthConfigService` now drive the JWT/session lifespans via `JWT_ACCESS_EXPIRATION`/`JWT_REFRESH_EXPIRATION` (duration strings like `"5m"`/`"7d"`, parsed with a new `duration.util.ts`), defaulting to the previous hardcoded 5m/7d if unset. `cookie.util.ts`'s cookie-option builders and `AuthService`'s JWT signing/refresh-token TTL calculations now take these values as parameters instead of importing hardcoded constants. GOTCHA: `@nestjs/config`'s latest major (12.x) is ESM-only (`"type": "module"`, no CJS build) and breaks Jest the same way the earlier `sanitize-html`/`htmlparser2` issue did - pinned to the last CJS-compatible major, `@nestjs/config@4.0.4` (exact version, no `^`), which still supports `@nestjs/common@^11`.

### Fixed

- **Active users could get logged out as "idle"**: `useIdleTimer`'s default event list now includes `click` and `scroll` (previously only `mousemove`/`mousedown`/`keydown`/`touchstart`/`wheel`), and `lib/api.ts` now dispatches a `finora:user-activity` window event on every API request, which `useIdleTimer` listens to alongside DOM events. This means any real interaction — including background fetches while the user is otherwise reading the screen — resets both the warning and logout timers.
...
- **`IdleWarningModal` never appeared with short timeouts, so the proactive heartbeat was removed**: the heartbeat (`use-session-heartbeat.ts`) and an intermediate shared-activity-clock rewrite (`lib/activity-tracker.ts`) both turned out to be unnecessary complexity chasing a problem the reactive 401-retry in `lib/api.ts` already solves (it transparently refreshes the access token on demand). Deleted both files and the `useSessionHeartbeat()` call in `DashboardLayout`. `useIdleTimer` is back to a single self-contained hook: local `useRef`s for the warning/logout `setTimeout`s, DOM+API activity listeners attached once per mount that debounce (300ms) before resetting them, and no shared/module-level state of any kind - eliminating the whole class of bug where something outside this hook could desync or reset its clock.
- **The warning modal could be dismissed by accident, without any explicit confirmation**: since passive DOM activity resets the idle clock, simply moving the mouse to reach the "Sigo aquí" button — or any other incidental movement/scroll while the modal was up — silently closed it and reset both timers, defeating the point of asking a financial-app user to confirm they're still there. `useIdleTimer` now tracks whether the warning has fired in an internal ref (`isWarningActiveRef`) and its activity listener returns immediately while it's set, ignoring every DOM/API activity event; the only way to dismiss the warning is now the "Sigo aquí" button, which calls `resetIdleTimer()` explicitly.
